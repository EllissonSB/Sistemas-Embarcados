...
static void uart_event_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (xQueueReceive(uart0_queue, (void *)&event, (TickType_t)portMAX_DELAY)) {
            ...
            //UART_PATTERN_DET
            case UART_PATTERN_DET:
                uart_get_buffered_data_len(EX_UART_NUM_2, &buffered_size);
                int pos = uart_pattern_pop_pos(EX_UART_NUM_2);
                //ESP_LOGI(TAG, "[UART PATTERN DETECTED] pos: %d, buffered size: %d", pos, buffered_size);
                if (pos == -1) {
                    // There used to be a UART_PATTERN_DET event, but the pattern position queue is full so that it can not
                    // record the position. We should set a larger queue size.
                    ...
                    uart_read_bytes(EX_UART_NUM_2, dtmp, pos, 100 / portTICK_PERIOD_MS);
                    uint8_t pat[PATTERN_CHR_NUM + 1];
                    memset(pat, 0, sizeof(pat));
                    uart_read_bytes(EX_UART_NUM_2, pat, PATTERN_CHR_NUM, 100 / portTICK_PERIOD_MS);
                    //ESP_LOGI(TAG, "read data: %s", dtmp);
                    //ESP_LOGI(TAG, "read pat : %s", pat);
                }
                break;
...
void app_main(void)
{
    ...
    //Set uart pattern detect function.
    uart_enable_pattern_det_baud_intr(EX_UART_NUM_2, '+', PATTERN_CHR_NUM, 9, 0, 0);
    //Reset the pattern queue length to record at most 20 pattern positions.
    uart_pattern_queue_reset(EX_UART_NUM_2, 20);
